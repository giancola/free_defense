...
class Cannon extends WeaponComponent {
  ...
  void fireBullet(Vector2 target) {
    BulletComponent bullet = BulletComponent(position: _bulletPosition(), size: weaponSetting.bulletSize)
      ..angle = barrel.angle
  ...
  void bulletExplosion(GameComponent enemy) {
    enemy.add(ExplosionComponent(position: enemy.size / 2, size: weaponSetting.explosionSize)
      ..animation = SpriteAnimation.spriteList(weaponSetting.explosionSprites, stepTime: 0.06, loop: false));
  }
}
